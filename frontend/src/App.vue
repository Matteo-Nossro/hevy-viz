  ...
  if (stored) {
    try {
      ...
      if (user) {
        ...
        // User supprimé de la BDD entre-temps → retour au picker
        clearStoredUsername()
      }
    } catch {
      // API down — on laisse passer au picker qui affichera l'erreur
    }
  }
...
<template>
  <!-- Spinner pendant vérification initiale du user -->
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="flex items-center justify-between h-14">
          ...
          <div class="flex items-center gap-3">
            <div v-if="workout.summary.value" class="text-xs text-text-muted hidden sm:block">
              {{ workout.summary.value.totalSessions }} séances ·
              {{ workout.summary.value.uniqueExercises }} exercices
            </div>
            <!-- Indicateur user + bouton changer -->
            <button
              @click="changeUser"
        ...
        <!-- Tabs -->
        <nav class="flex gap-0 overflow-x-auto -mb-px" v-if="workout.loaded.value">
          <button
            v-for="tab in tabs"
            :key="tab.id"
